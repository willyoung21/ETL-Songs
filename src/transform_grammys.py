def transform_db():
    ...
    def save_progress(idx):
        with open('/home/willyb/airflow/last_processed_index.txt', 'w') as f:
            f.write(str(idx))
    ...
    def load_progress():
        try:
            ...
            return 0  # Si no existe, comenzar desde el índice 0

    # Cargar el índice del último procesamiento
    ...
    for idx, row in df_nulos_artist.iterrows():
        # Si el índice ha sido procesado, continuar
    ...
    def get_spotify_id(nominee, artist):
        try:
            ...
            if e.http_status == 429:
                ...
                return get_spotify_id(nominee, artist)  # Vuelve a intentar
            else:
                print(f"Error retrieving data for nominee: {nominee}, artist: {artist}: {e}")
                return np.nan

        except ReadTimeout:
            print(f"Tiempo de espera excedido para nominee: {nominee}, artist: {artist}. Reintentando en 10 segundos...")
            time.sleep(10)
            return get_spotify_id(nominee, artist)
